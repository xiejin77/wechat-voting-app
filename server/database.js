...
function init() {
  ...
  // 创建索引以提高查询性能
  // 为votes表的创建时间创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_votes_created_at ON votes (createdAt)`);
  
  // 为vote_options表的voteId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_vote_options_vote_id ON vote_options (voteId)`);
  
  // 为whitelist表的userId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_whitelist_user_id ON whitelist (userId)`);
  
  // 为votes_records表的voteId和userId创建复合索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_votes_records_vote_user ON votes_records (voteId, userId)`);
  
  // 为votes_records表的voteId和optionId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_votes_records_vote_option ON votes_records (voteId, optionId)`);
  
  // 为vote_results表的voteId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_vote_results_vote_id ON vote_results (voteId)`);
  
  // 为vote_statistics表的voteId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_vote_statistics_vote_id ON vote_statistics (voteId)`);
  
  // 为obfuscation_logs表的voteId创建索引
  db.run(`CREATE INDEX IF NOT EXISTS idx_obfuscation_logs_vote_id ON obfuscation_logs (voteId)`);
